...
data <- select(read.csv("/home/poom/Desktop/airbnb/listings.csv"), id, listing_url, description, house_rules, 
...
dta$host_listings_count <- data$host_listings_count #theres outlier
...
dta$moderator <- dta$host_is_superhost * dta$num_competitors_zip
est <- lm(logit_percent_perf1m ~ host_is_superhost + num_competitors_zip + moderator +
