...
data = data[['listing_id', 'id', 'date', 'reviewer_id', 'country']].copy()
countries = ["United States", "China", "Hong Kong", "Taiwan", "Japan", "South Korea"]
data = data[data.country.str.contains('|'.join(countries))]
data['usvasia'] = np.where(data['country'].str.contains('United States', na=False), 1, 0)
...
df = df[['id', 'date', 'availability_30', 'host_is_superhost', 'review_scores_rating', 'number_of_reviews', 'price', 'id_date']]
df.date.unique()
...
df['host_is_superhost'] = np.where(df['host_is_superhost']=="t", 0, 1)
